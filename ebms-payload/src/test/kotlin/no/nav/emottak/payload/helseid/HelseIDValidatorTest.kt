...
internal class HelseIDValidatorTest {
    ...
    @Test
    fun `tester noe noe`() {
        val norskHelseNettOrgNr = ""
        val issuer = ""
        val validator = HelseIDValidator(ISSUER, ORGNO)
    }
    ...
    fun `validate helseID wrong algorithm`() {
        val jwk = OctetSequenceKeyGenerator(256)
            ...
            .issueTime(Date())
            .generate()
        validateHomeMadeHelseId(validator, jwk = jwk, scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
    ...
    @Test
    fun `validate helseID with EC jwk`() {
        // this fails with NoSuchElementException because we expect to find
        // the certificate used to sign the token in the list of certificates
        // this must be rewritten when we change to validate signature from JWKs
        val jwk = ECKeyGenerator(Curve.P_256)
            ...
            .generate()
        validateHomeMadeHelseId(validator, jwk = jwk, scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
            audience = HelseIDValidator.SUPPORTED_AUDIENCE.first(), errMsg = "no certificate to verify token with given",
            algo = JWSAlgorithm.ES256)
    }

    @Test
    fun `validate helseID with wrong scope`() {
        validateHomeMadeHelseId(validator, scope = "e-hacker:hackhack",
            audience = HelseIDValidator.SUPPORTED_AUDIENCE.first(),
    ...
    fun `validate helseID with missing audience`() {
        validateHomeMadeHelseId(validator, scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
    ...
    fun `validate helseID with wrong issuer`() {
        validateHomeMadeHelseId(
            HelseIDValidator("https://foo.bar", ORGNO), scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
            audience = HelseIDValidator.SUPPORTED_AUDIENCE.first(), "invalid issuer https://helseid-sts.test.nhn.no")
    }
    ...
    fun `validate helseID with right type in header`(type: JOSEObjectType) {
        validateHomeMadeHelseId(
            HelseIDValidator("https://foo.bar", ORGNO), scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
            audience = HelseIDValidator.SUPPORTED_AUDIENCE.first(), type = type,
    ...
    fun `validate helseID with wrong type in header`() {
        validateHomeMadeHelseId(
            HelseIDValidator("https://foo.bar", ORGNO), scope = HelseIDValidator.SUPPORTED_SCOPES.first(),
            audience = HelseIDValidator.SUPPORTED_AUDIENCE.first(), type = JOSEObjectType("foo"),
            errMsg = "Unsupported token type foo")
    }

    @Test
    fun `validate helseID with wrong orgno`() {
        val validator = HelseIDValidator(ISSUER, "987654321")
        val b64 = ""
        val timeStamp = ZonedDateTime.of(2019, 5, 21, 10, 22, 0, 0, ZoneId.systemDefault())
        val ex = assertThrows<RuntimeException> {
            validator.validate(b64, timeStamp = timeStamp, certificates = certsFromM1)
        }
        assertThat(ex.message,
            `is`("Signer certificate organization number 994598759 is not the approved organization number"))
    }

    @Suppress("LongParameterList")
    private fun validateHomeMadeHelseId(validator: HelseIDValidator, scope: String,
                                        audience: String,
                                        errMsg: String? = null,
        ...
        val b64 = Base64.getEncoder().encodeToString(
            if (jwk == null) {
                helseIDCreator.getToken(alias = "docsigner", pid = "01010000110",
                scope = scope, audience = audience, algo = algo, type = type).toByteArray()
            } else {
                helseIDCreator.getToken(jwk = jwk, pid = "01010000110",
                    scope = scope, audience = audience, algo = algo, type = type).toByteArray()
            }
        )
    ...
    fun validateHelseID2() {
        ...
        TimeUnit.MILLISECONDS.sleep(20)
        val timeStamp = ZonedDateTime.now()
        validator.validate(token, timeStamp, certificates = setOf(certificate))
    }
    ...
    fun validateHelseIDwrongTimestamp(ts: String, msg: String) {
        val b64 = ""
        val certificate = X509Utils.loadCertificate(ResourceUtil.getByteArrayClasspathResource("helseid/certificates/cert.personal.commfides.cer"))
        val t: Throwable = assertThrows<RuntimeException> { validator.validate(b64,
            timeStamp = ZonedDateTime.parse(ts), certificates = setOf(certificate)) }
    ...
    fun validateHelseIDwrongTimestampWithLargeAllowedClockSkew(ts: String) {
        ...
        val t: Throwable = assertThrows<RuntimeException> {
            validator2.validate(b64, timeStamp = ZonedDateTime.parse(ts), certificates = setOf(certificate))
        }
    ...
    private fun verifier(jwk: JWK): JWSVerifier =
        when(jwk.algorithm) {
            ...
            JWSAlgorithm.PS256, JWSAlgorithm.PS384, JWSAlgorithm.PS512 -> RSASSAVerifier(jwk.toRSAKey().toRSAPublicKey())
    ...
    companion object {
        ...
        private val certsFromM1 = listOf("",
            ...
            "",
            "",
            "",
            "")
...
internal fun timestampAndErrorPairs(): List<Arguments> {
    return listOf(Arguments.of("2018-12-10T13:51:00.000+01:00", "Timestamp (Mon Dec 10 13:51:00 CET 2018) is before token not-before time (Tue Dec 11 12:52:29 CET 2018)"),
        Arguments.of("2018-12-12T13:51:00.000+01:00", "Timestamp (Wed Dec 12 13:51:00 CET 2018) is after token expiry time (Tue Dec 11 13:52:29 CET 2018)"))
}
