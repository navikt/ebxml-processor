    ...
    internal suspend fun incomingRetryEval(
        ...
        when (decision) {
            ...
            RetryDecision.TTL_EXPIRED -> {
                ...
                log.warn(payloadMessage.marker(), "MESSAGE_GIVEN_UP: TTL expired: incoming with key ${record.key()} at offset ${record.offset()}, retried $retryCount times")
            }
            RetryDecision.MAX_RETRIES_EXCEEDED -> {
                ...
                log.warn(payloadMessage.marker(), "MESSAGE_GIVEN_UP: Max Retry Exceeded: incoming with key ${record.key()} at offset ${record.offset()}, retried $retryCount times")
            }
            RetryDecision.NO_RETRY -> {
                ...
                log.warn(payloadMessage.marker(), "MESSAGE_GIVEN_UP: No Retry: incoming with key ${record.key()} at offset ${record.offset()}")
            }
        }
