...
class SertifikatValideringTest : FunSpec({
    ...
    context("Sertifikatsjekker med mocked CRLChecker") {
        ...
        test("Selvsignert sertifikat feiler") {
            ...
            exception.message shouldBe "Sertifikat er selvsignert"
        }

        test("Expired sertifikat feiler") {
            val exception = shouldThrow<CertificateValidationException> {
                sertifikatValidering.validateCertificate(TestUtil.expiredCertificate)
            }
            exception.message shouldStartWith "Sertifikat utløpt"
        }

        test("Gyldig sertifikat validerer OK") {
        ...
        test("Gyldig sertifikat med ukjent trust chain feiler") {
            val exception = shouldThrow<CertificateValidationException> {
                sertifikatValidering.validateCertificate(TestUtil.validCertificate)
            }
            exception.message shouldBe "Sertifikatvalidering feilet"
        }

//        test("Revokert sertifikat feiler") {
//            val exception = shouldThrow<CertificateValidationException> {
